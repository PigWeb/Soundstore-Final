...
<body>
	...
	<div id="encabezado"> 
		...
		<div id="contenido">
				...
				<div id="usuario">
				<?php 
						//llama datos ingresados en el formulario y los asigna a variables locales			

						$nombre = $_POST["nombre"];
						$edad = $_POST["edad"];
						$email = $_POST["mail"];
						$dire = $_POST["direccion"];
						$locali = $_POST["localidad"];
						...
						if($nombre == "" or $email == "" or $dire == "" or $edad=="" or $p1=="" or $p2==""){ 
						$funcion_validar[] = "Debes completar todos los campos"; 
						} 

						if(strlen($nombre)<5){ 
						$funcion_validar[] ="El nombre debe tener menos de 5 caracteres"; 
						}

						if(is_numeric($edad)==0){
						$funcion_validar[] ="La edad debe contener solo numeros";
						}

						if($edad<18){
						$funcion_validar[]="Debe ser mayor de 18 a&ntilde;os";
						}

						if($edad>99){
						$funcion_validar[]="Ingrese una edad valida";
						}

						if(!filter_var($email,FILTER_VALIDATE_EMAIL )){ 
						$funcion_validar[] = "El email no es valido"; 
						} 

						if($p1 != $p2){ 
						$funcion_validar[] = "Los passwords deben coincidir"; 
						} 

						if(empty($funcion_validar)){
						//Invocamos el procedimiento almacenado, incluyendo los valores
						$con="CALL adduser('$email','$nombre','$edad','$dire','$locali','$pcia','$p1','$admin')";
						require("mysql.php");
						if($consulta!='0')
							echo "usuario correctamente cargado";
						mysqli_close($conexion); 
						echo "<br>Se ha creado correctamente un nuevo usuario!";
