...
<body>
	...
	<div id="contenido">
				...
				<div id="carrito">
				<div class="titulo3">COMPRA REALIZADA</div>
<?php
		$usr=$_SESSION['nom'];
		$mail=$_SESSION['mail'];
		$total=0;
		$tabla="<table border='1' style='margin: 0 auto'>
			<tr>
			<th>Nombre</th>
			<th>Cantidad</th>
			<th>Precio</th>
  ...
  $total2=0;		
  $ar=fopen("../../fpdf/facturacompra.txt","w") or
  ...
  fputs($ar,chr(13).chr(10));
  fputs($ar,chr(13).chr(10));
  fputs($ar,"COMPRA REALIZADA");
  fputs($ar,chr(13).chr(10));
  fputs($ar,"*No valido como factura de compra");
  fputs($ar,chr(13).chr(10));
  fputs($ar,"USUARIO: ");
  fputs($ar,$usr);
  fputs($ar,chr(13).chr(10));
 ...
 for($i=0;$i<count($arreglo);$i++){ 
  fputs($ar,chr(13).chr(10));
  fputs($ar,"NOMBRE DEL DISCO: ");
  fputs($ar,$arreglo[$i]['Nombre']);
  fputs($ar,chr(13).chr(10));
  fputs($ar,"CANTIDAD: ");
  fputs($ar,$arreglo[$i]['Cantidad']);
  fputs($ar,chr(13).chr(10));
  fputs($ar,"PRECIO: $");
  fputs($ar,$arreglo[$i]['Precio']);
  fputs($ar,chr(13).chr(10));
  fputs($ar,"SUBTOTAL: $");
  fputs($ar,$arreglo[$i]['Cantidad']*$arreglo[$i]['Precio']);
  fputs($ar,chr(13).chr(10));
  $total2=$total2+($arreglo[$i]['Cantidad']*$arreglo[$i]['Precio']);
   }
  fputs($ar,chr(13).chr(10));
  fputs($ar,"TOTAL FINAL: $");
  fputs($ar,$total2);
  fputs($ar,chr(13).chr(10));
  fputs($ar,"--------------------------------------------------------");
  fputs($ar,chr(13).chr(10));
...
?>
  ...
  <br/>
  <br/>
  <button onclick="location.href='../../fpdf/descargar.php'" target="_blank"><span class="glyphicon glyphicon-file"></span>Descargar Factura en PDF</button>
