<?php 						
session_start(); 
$con="CALL selalldisco";	
		include "../mysql.php";
		while ($f=mysqli_fetch_array($consulta)) {
				$Iddisco=$f['iddisco'];
				$Idartista=$f['id_artista'];
				$Nombredisco=$f['nombre_d'];
				$Artista=$f['artista'];
				$Genero=$f['genero'];
				$Precio=$f['precio'];
				$Formato=$f['formato'];
				$Portada=$f['portada'];
				$Temap=$f['tema_preview'];
				$Ntemap=$f['nombretpre'];
				$arreglo[]=array('Id'=>$Iddisco,
...
<body>
	...
	<div id="encabezado"> 
		...
		<div id="contenido">
				...
				<div id="resultados_disco"><br/><br/>
					<div class="titulo3">PANEL DE CONTROL - DISCOS</div>
					<br>
					<?php 
							echo "<form action='adddisco.php'>
    							<input type=submit value='Agregar un disco nuevo' name='buy'>
								</form>";
							echo "<br/>";
						...
						for($i=0;$i<count($arreglo);$i++){
							$tabla2='<tr>
								  <td>'.$arreglo[$i]['Id'].'</td>
								  <td>'.$arreglo[$i]['Id_Artista'].'</td>
								  <td>'.$arreglo[$i]['Nombre_disco'].'</td>
								  <td>'.$arreglo[$i]['Artista'].'</td>
								  <td>'.$arreglo[$i]['Genero'].'</td>
								  <td>'.$arreglo[$i]['Precio'].'</td>
								  <td>'.$arreglo[$i]['Formato'].'</td>
								  <td>'.$arreglo[$i]['Portada'].'</td>
								  <td>'.$arreglo[$i]['Tema_Preview'].'</td>
								  <td>'.$arreglo[$i]['Nombre_Preview'].'</td>
								  <td><a href="eliminar-disco.php?Id='.$arreglo[$i]['Id'].'">
				    					<img src="../../img/delete.png" width="20px" height="20px"/></a>
				    				  <a href="edit-disco.php?Id=' .$arreglo[$i]['Id'].'">
