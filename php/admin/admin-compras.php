...
$con="CALL selallcomp";	
		...
		while ($f=mysqli_fetch_array($consulta)) {
				$Idcompra=$f['idc'];
				$Idventa=$f['numeroventa'];
				$Img=$f['imagen'];
				$Nombredisco=$f['nombre'];
				$Precio=$f['precio'];
				$Cantidad=$f['cantidad'];
				$Subtotal=$f['subtotal'];
				$Iddisco=$f['iddisco'];
				$Mail=$f['mailusr'];
				$arreglo[]=array('Idc'=>$Idcompra,
							'Idventa'=>$Idventa,
							'Portada'=>$Img,
							'Disco'=>$Nombredisco,
							'Precio'=>$Precio,
	...
	<div id="encabezado"> 
		...
		<div id="contenido">
				...
				<div id="resultados_disco"><br/><br/>
					<div class="titulo3">INFORME DE COMPRAS</div>
					<br>
					<?php 
						...
						for($i=0;$i<count($arreglo);$i++){
							$tabla2='<tr>
								  <td>'.$arreglo[$i]['Idc'].'</td>
								  <td>'.$arreglo[$i]['Idventa'].'</td>
								  <td>'.$arreglo[$i]['Disco'].'</td>
								  <td>'.$arreglo[$i]['Precio'].'</td>
								  <td>'.$arreglo[$i]['Cant'].'</td>
								  <td>'.$arreglo[$i]['Subtotal'].'</td>
								  <td>'.$arreglo[$i]['Idisco'].'</td>
								  <td>'.$arreglo[$i]['Usr'].'</td>
								  </tr>';
							echo $tabla2;
								} 
