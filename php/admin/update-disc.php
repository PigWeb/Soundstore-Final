...
<body>
	...
	<div id="encabezado"> 
		...
		<div id="contenido">
				...
				<div id="resultados_admin"><br/><br/>
					<div class="titulo3">PANEL DE CONTROL - DISCOS</div>
					<br>
					<?php 
						$Id = $_POST["iddisco"];
						$ndisco = $_POST["nombred"];
						...
						if ($_FILES["img"]["size"]!=0)
								{
									move_uploaded_file($_FILES["img"]["tmp_name"], "../../files/$nomimg");
								}	

						if ($_FILES["mp3"]["size"]!=0)
								{
									move_uploaded_file($_FILES["mp3"]["tmp_name"], "../../files/$nmp3");
								}	
						

						$funcion_validar = array(); 

						if($Id =="" or $ndisco == "" or $artist == "" or $genero=="" or $precio=="" or $formato=="" or $nomimg=="" or $nmp3=="" or $npre==""){ 
						$funcion_validar[] = "Debes completar todos los campos"; 
						} 

						if(is_numeric($precio)==0){
						$funcion_validar[] ="El precio debe contener solo numeros";
						}

						if ($_FILES["img"]["size"]==0){
						$funcion_validar[] ="El archivo de imagen no fue subido correctamente";
						}

						if ($_FILES["mp3"]["size"]==0){
						$funcion_validar[] ="El archivo de sonido no fue subido correctamente (debe ser de menos de 10mb)";
						}
												
						if(empty($funcion_validar)){
						//Invocamos el procedimiento almacenado, incluyendo los valores
						$con="UPDATE `store`.`disco` SET `nombre_d`='$ndisco', 
						`artista`='$artist', `genero`='$genero', `precio`='$precio',`formato`='$formato', `portada`='$nomimg', `tema_preview`='$nmp3', `nombretpre`='$npre'
						WHERE `iddisco`='$Id'";
						
						require("../mysql.php");
						if($consulta!='0')
							echo "Los datos del disco han sido actualizados";
						mysqli_close($conexion); 
						
						}
						else{ 
						echo "La informacion no ha sido actualizada. <br/>";
						foreach($funcion_validar as $r){ 
						echo "Errores: ".$r."<br>";
						} 
